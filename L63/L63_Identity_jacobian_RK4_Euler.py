...
for seed in range(5):
    ...
    params['model_save_file_name'] = 'L63_model_error_identity_jacobians_dopri5_Euler_dt02_seed_'+str(seed)+'.pt'
    params['device']             = 'cpu'
    params['seed']               = seed
    ...
    def time_stepper(x0,dt,model):
        ...
        return S[-1,:]#pred#
    def forward_black_box_model(model, dt, n, x0):
        pred = [x0]
        comp_to = [x0[:-n]]
        for i in range(n):# loop over sequence
            pred_batch = torch.zeros_like(pred[-1])
            for k in range(x0.shape[0]):# loop over batch 
                pred_batch[k,:] = torch.from_numpy(time_stepper(pred[-1][k,:].detach().numpy(),dt,model))
    ...
    def train_model(model, optimizer, scheduler, device, num_epochs=25, dt = params['dt_integration'], seq_size = params['seq_size'], nb_part = params['nb_part'], grad_list = grad_list, param_list = param_list):
        ...
        #                    print('max inps :' ,inputs.max())
                            pred_mdl = forward_black_box_model(model, dt, seq_size, x_data)
                            ...
                            if phase == 'train':
                                ...
                                gradss = compute_and_set_gradients(model,pred_mdl,dt,grad0)
                                grad_list, params_list = store_params(model,grad_list,param_list)
                                optimizer.step()
                                loss_batch_train.append(loss.detach().numpy())
                            else:
                                loss_batch_test.append(loss.detach().numpy())                            
                        # statistics
    ...
    model_best_valid, model, grad_list, param_list = train_model(M_theta, optimizer_ft, exp_lr_scheduler, device = params['device'], num_epochs=600,  dt = params['dt_integration'], seq_size = params['seq_size'], nb_part = params['nb_part'], grad_list = grad_list, param_list = param_list)
