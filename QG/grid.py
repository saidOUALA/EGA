...
class TwoGrid:
  ...
  def reduce(self, y):
    ...
    z_new = torch.zeros([self.Ny, self.dk], dtype=torch.complex128, requires_grad=False).to(self.device)

    z_new[:int(self.Ny / 2), :self.dk] = y[:int(self.Ny / 2), :self.dk]
    z_new[int(self.Ny / 2):self.Ny, :self.dk] = y[y_r[0] - int(self.Ny / 2):y_r[0], :self.dk]
  ...
  def increase(self, y):
    ...
    z_new[:int(y_r[0] / 2), :y_r[1]] = y[:int(y_r[0] / 2), :y_r[1]]
    z_new[self.Ny - int(y_r[0] / 2):self.Ny, :y_r[1]] = y[int(y_r[0] / 2):y_r[0], :y_r[1]]
